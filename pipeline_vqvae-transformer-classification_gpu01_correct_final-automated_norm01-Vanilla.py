...
n_total = masked_train_quantizes.shape[0]
idx_all = np.arange(n_total)
tr_idx, val_idx = train_test_split(
    idx_all, test_size=0.10, random_state=42, shuffle=False
)

x_tr  = masked_train_quantizes[tr_idx]
y_tr  = train_indices_label[tr_idx]
x_val = masked_train_quantizes[val_idx]
...
#        train(i, train_dataloader, model, optimizer, scheduler, device)
    ...
    if dist.is_primary():
        ckpt_path = f"/local/altamabp/checkpoint/distil/80x80_100ClassImagenet_flat_144x456codebook_75mask_epoch{str(j).zfill(3)}_vanilla.pt"
