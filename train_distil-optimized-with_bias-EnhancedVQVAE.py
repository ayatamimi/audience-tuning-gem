...
os.chdir('/home/altamabp/audience-tuning-gem')
...
def main(args):
    ...
    val_labels= np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/val_labels.npy')
    print('val_labels shape: ',val_labels.shape)
    
    val_indices = np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/val_latent_space_vqvae_80x80_codebook_16x124.npy')
    print('val_indices shape: ',val_indices.shape)
 
    val_quantizes = np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/val_codebook_vqvae_80x80_codebook_16x124.npy')
    ...
    train_labels = np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/train_labels.npy')
    
    train_indices = np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/train_latent_space_vqvae_80x80_codebook_16x124.npy')

    train_quantizes = np.load('./checkpoint_correct/vqvae/data_EnhancedVQVAE/train_codebook_vqvae_80x80_codebook_16x124.npy')
...
############ Data prepration and masking/ train set##############
    ...
    masked_train_quantizes = np.copy(train_quantizes)
...
############### Data prepration and masking/ validation set#####################
    ...
    masked_val_quantizes = np.copy(val_quantizes)
    masked_val_quantizes[mask_val] = mask_token
    val_indices_label = np.copy(val_indices)
    #val_indices_label[~mask_val] = mask_token_label
    masked_val_quantizes = torch.from_numpy(masked_val_quantizes)
    ...
    masked_exp_mask_feat_val_quantizes = attach_bias_mask_feat (val_labels, masked_val_quantizes, mask_val, mask_perc)
    print('masked_exp_mask_feat_val_quantizes.shape: ',masked_exp_mask_feat_val_quantizes.shape)
...
################Create Data loaders###################
    ...
    val_data = CustomDataset(masked_exp_mask_feat_val_quantizes, val_indices_label, mask_perc,n_val_samples,n_val_tokens, mask_token)
...
#####################Model Config###########################
    ...
    for i in range(args.epoch):
        ...
        if dist.is_primary():
            torch.save(model.state_dict(), f"./checkpoint_correct/distil/80x80_100_UTKFace_flat_16x124codebook_50mask_epoch{str(j).zfill(3)}_without-ignore-index.pt")
