# -*- coding: utf-8 -*-
"""
Created on Sat Oct 25 16:40:00 2025
...
def train(epoch, train_loader, val_loader, model, optimizer, scheduler, device):
    if dist.is_primary():
         ...
         if dist.is_primary():
             ...
             if i % 9000 == 0:
                 ...
                 utils.save_image(
                     ...
                     f"image_correct/sample/flat_vqvae_80x80codebook_{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.png",
                     nrow=sample_size,
                     normalize=True,
                     value_range=(-1, 1),
                 )
    ...
    # -------------------------
    # Validation loop (no grad, no backprop, kept separate from train loss)
    # -------------------------
    model.eval()
    val_mse_sum = 0.0
    val_mse_n = 0
    with torch.no_grad():
        for img_val, _ in val_loader:
            ...
            out_val, latent_loss_val, diversity_loss_val, _ = model(img_val)
            recon_loss_val = criterion(out_val, img_val)

            # Accumulate ONLY for reporting — do not backprop and do not mix into train loss
            part_val_sum = recon_loss_val.item() * img_val.shape[0]
            part_val_n = img_val.shape[0]
            comm_val = {"mse_sum": part_val_sum, "mse_n": part_val_n}
            comm_val = dist.all_gather(comm_val)

            for part in comm_val:
                val_mse_sum += part["mse_sum"]
                val_mse_n += part["mse_n"]

    if dist.is_primary():
        avg_val_mse = val_mse_sum / max(1, val_mse_n)
        run["val/vqvae-mse"].log(avg_val_mse)
        # Optional: log component-wise (not used for optimization)
        # run["val/vqvae-latent"].log(latent_loss_val.mean().item())
        print(f"[Validation] epoch: {epoch + 1}; avg val mse: {avg_val_mse:.5f}")
    model.train()
...
def main(args):
    ...
    dataset = datasets.ImageFolder("UTKFace_dataset_subset_150000_structured", transform=transform)
    data_loader = DataLoader(dataset, batch_size=batchsize_modified// args.n_gpu, shuffle=True, num_workers=12)

    # Validation dataset/loader (separate folder)
    val_dataset = datasets.ImageFolder("/local/altamabp/UTKFace_dataset_subset_10000_structured", transform=transform)
    val_loader = DataLoader(val_dataset, batch_size=batchsize_modified// args.n_gpu, shuffle=False, num_workers=12)
