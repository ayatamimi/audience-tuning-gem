...
def _latest_ckpt_in_distil_dir():
    ckpt_dir = "local/altamabp/mdl_checkpoints"
...
def train(epoch, loader, model, optimizer, scheduler, device, val_loader=None):
    ...
    if _is_primary() and "run" in globals() and run is not None:
        run["train/vqve-loss"].log(train_loss_avg, step=epoch + 1)
...
parser.add_argument(
    ...
    type=str,
    default="/local/altamabp/mdl_checkpoints/distilBERT_weights.pt",
...
def fit_distil_with_embeds(
    ...
    ckpt_dir: str = "mdl_checkpoints",
    resume_weights: str = "distilBERT_weights.pt",
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(model.state_dict(), "distilBERT_weights.pt")  # best weights
...
torch.cuda.set_device(1)  # Set default CUDA device to 3
torch.cuda.empty_cache()
...
for ep in range(start_epoch + 1, start_epoch + 1 + args.epoch):
    ...
    if 'run' in globals() and run is not None and is_primary():
        run["train/transformer-epoch"].log(ep)
...
parser.add_argument('--ckpt_vqvae', type=str, default="/local/altamabp/checkpoint/vqvae/model_epoch100_flat_vqvae80x80_144x456codebook_fullTrain.pth")
parser.add_argument('--ckpt_distil_combined', type=str, default="/local/altamabp/mdl_checkpoints/distilBERT_weights.pt")
...
for x in tqdm(range(quantizes.shape[0]), desc="Processing", unit="img"):
    ...
    for perc in mask_percentages:
        with torch.no_grad():
            ...
            masked_exp_train_quantizes = torch.cat([q_masked, one_hot_labels], dim=2)
            ...
            # 3) Concatenate
            masked_exp_mask_feat_train_quantizes = torch.cat([masked_exp_train_quantizes, mask_feat], dim=-1) 
